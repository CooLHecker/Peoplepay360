...
from app.models import Contract, ContractStatus, Employee, RoleName, User
...
_READ_ROLE_NAMES = set(_READ_ROLES)
...
def _assert_can_view(user: User, contract: Contract) -> None:
    """Payroll/HR roles can view any contract; a plain employee can
    only ever view their own (matched via their linked Employee
    record, never a client-supplied employee_id) — same self-service
    pattern as payslips (see app/api/v1/endpoints/payslips.py)."""
    role_names = {r.name for r in user.roles}
    if role_names & _READ_ROLE_NAMES:
        return
    if user.employee_id is not None and user.employee_id == contract.employee_id:
        return
    raise HTTPException(
...
def list_contracts(
    ...
    role_names = {r.name for r in user.roles}
    is_payroll = bool(role_names & _READ_ROLE_NAMES)
    ...
    if is_payroll:
        ...
        # Non-payroll users (a plain employee viewing their own
        # dashboard) can only ever list their own contracts,
        # regardless of what employee_id (if any) was requested.
        if user.employee_id is None:
            return []
        query = query.filter(Contract.employee_id == user.employee_id)
...
def get_contract(
    ...
    _assert_can_view(user, contract)
    return _contract_response(contract)

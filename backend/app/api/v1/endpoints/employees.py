...
def _employee_response(employee: Employee) -> EmployeeResponse:
    ...
    return EmployeeResponse(
        ...
        positionId=employee.job_position,
...
def create_employee(payload: EmployeeCreate, db: Session = Depends(db_dependency)) -> EmployeeResponse:
    employee = Employee(
        ...
        # New employees default to Gandhinagar when no location is
        # supplied (rather than leaving it blank or hardcoding
        # Bengaluru in the UI, as before).
        location=payload.location or "Gandhinagar, India",
...
def update_employee(employee_id: int, payload: EmployeeUpdate, db: Session = Depends(db_dependency)) -> EmployeeResponse:
    ...
    employee.job_position = payload.job_position

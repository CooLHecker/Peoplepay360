...
export default function TimeOffPage() {
  ...
  useEffect(() => {
    ...
    if (admin) {
      ...
      void getApiOr<Employee[]>("/employees/", demoEmployees).then((result) => { if (!cancelled) setEmployees(result); });
    } else {
  ...
  return <div className="mx-auto max-w-[1440px]">
    <PageHeading
      ...
      action={<button onClick={() => setModalOpen(true)} className="inline-flex items-center gap-2 rounded-lg bg-[#714b67] px-4 py-2.5 text-sm font-bold text-white"><Plus size={17} />Request time off</button>}
    />
...
function RequestTimeOffModal({ types, typesError, employees, admin, onClose, onCreated }: {
  types: TimeOffType[];
  typesError: string;
  employees: Employee[];
  admin: boolean;
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!Number.isFinite(timeOffTypeIdNumber)) { setError(typesError || "This time off type list looks out of date — check the API connection and reopen this form."); return; }
  ...
  return <div className="fixed inset-0 z-40 grid place-items-center bg-[#25212a]/30 p-4">
    <section className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
      ...
      {typesError && <p className="mt-4 rounded-lg bg-[#fff7e7] p-3 text-sm font-semibold text-[#a36b12]">Time off types failed to load: {typesError}</p>}
      <form onSubmit={submit} className="mt-5 space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button disabled={saving} className="rounded-lg bg-[#714b67] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{saving ? "Submitting..." : "Submit request"}</button>
        </div>
      </form>
    </section>
  </div>;

...
export function Toolbar({ placeholder = "Search records", onSearch, sortOptions, sortValue, onSortChange }: { placeholder?: string; onSearch: (value: string) => void; sortOptions?: { label: string; value: string }[]; sortValue?: string; onSortChange?: (value: string) => void }) {
  return <div className="mb-4 flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#b2a6ae]" /><input className="h-11 w-full rounded-lg border border-[#e6e0e5] bg-white pl-10 pr-4 text-sm outline-none transition placeholder:text-[#b2a6ae] focus:border-[#714b67] focus:ring-4 focus:ring-[#714b67]/10" placeholder={placeholder} onChange={(event) => onSearch(event.target.value)} /></label>{sortOptions && onSortChange ? <label className="relative"><select className="h-11 w-full cursor-pointer appearance-none rounded-lg border border-[#e6e0e5] bg-white py-2 pl-4 pr-10 text-sm font-bold text-[#756c75] outline-none transition hover:border-[#714b67] hover:text-[#714b67] focus:border-[#714b67] focus:ring-4 focus:ring-[#714b67]/10 sm:w-auto" value={sortValue} onChange={(event) => onSortChange(event.target.value)} aria-label="Sort list">{sortOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><SlidersHorizontal size={16} className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-[#b2a6ae]" /></label> : <button className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-[#e6e0e5] bg-white px-4 text-sm font-bold text-[#756c75] hover:border-[#714b67] hover:text-[#714b67]"><SlidersHorizontal size={16} /> Filters</button>}</div>;
}

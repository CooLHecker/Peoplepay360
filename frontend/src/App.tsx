...
function RequireSession({ children, admin = false }: { children: ReactElement; admin?: boolean }) {
  ...
  if (!session && !localStorage.getItem("peoplepay_demo_session")) return <Navigate to="/login" state={{ from: location.pathname }} replace />;
  if (admin && session && !isAdmin(session.roles)) return <Navigate to="/my-dashboard" replace />;
  return children;
}
